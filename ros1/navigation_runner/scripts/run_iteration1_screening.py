#!/usr/bin/env python3
"""Sequentially run or resume iteration-1 tunnel screening candidates.
...
class Candidate:
    ...
    recover_forward_speed: Optional[float] = None


CANDIDATES = (
    Candidate(
        name="hold_baseline",
        output_dir="/root/results/iter1_hold030_seed5716",
        safety_min_dist=0.30,
        safety_mode="hold",
    ),
    Candidate(
        name="recover_default",
        output_dir="/root/results/iter1_recover030_default_seed5716",
        safety_min_dist=0.30,
        safety_mode="recover",
        recover_speed=0.35,
        recover_forward_speed=0.15,
    ),
    Candidate(
        name="recover_conservative",
        output_dir="/root/results/iter1_recover035_conservative_seed5716",
        safety_min_dist=0.35,
        safety_mode="recover",
        recover_speed=0.30,
        recover_forward_speed=0.10,
    ),
    Candidate(
        name="recover_fast",
        output_dir="/root/results/iter1_recover030_fast_seed5716",
        safety_min_dist=0.30,
        safety_mode="recover",
        recover_speed=0.45,
        recover_forward_speed=0.20,
    ),
)
...
candidates = json.loads(os.environ["ITER1_CANDIDATES_JSON"])
...
def candidate_output_dir(candidate, args):
    ...
    if basename.startswith("iter1_"):
        basename = basename[len("iter1_"):]
...
def build_batch_argv(candidate, args, resume):
    ...
    if candidate.safety_mode == "recover":
        argv.extend(
            [
                "--safety-recover-speed",
                format_float(candidate.recover_speed),
                "--safety-recover-forward-speed",
                format_float(candidate.recover_forward_speed),
            ]
        )
    if resume:
...
def build_batch_inner(candidate, args):
    ...
    lines = [
        f"echo '=== iteration-1 candidate: {candidate.name} ==='",
...
def build_summary_inner(candidates, args):
    ...
    return with_ros_setup(
        [
            f"ITER1_CANDIDATES_JSON={shlex.quote(payload)} python3 - <<'PY'",
            SUMMARY_CODE,
            "PY",
        ]
    )
...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Run/resume iteration-1 tunnel screening candidates sequentially."
    )
    ...
    parser.add_argument(
        "--output-tag",
        default="",
        help="Optional output prefix, e.g. quick1 -> /root/results/quick1_hold030_seed5716",
    )
...
def main(argv=None):
    ...
    if not failures and (args.run or args.print_inner_command):
        run_inner(build_summary_inner(candidates, args), args, "summarize iteration-1", allow_failure=True)
    elif not failures:
