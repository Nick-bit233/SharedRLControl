# --- 使用isaaclab的AppLauncher启动 Isaac Sim ---
from isaaclab.app import AppLauncher
import argparse

parser = argparse.ArgumentParser(description="Verify IPC Controller (Tunnel Environment)")
parser.add_argument("--config", type=str,
                    default=None,
                    help="Path to IPC config YAML (default: ipc_config.yaml in same dir)")
parser.add_argument("--usermodel", action="store_true",
                    help="Use UserModelTunnel to generate human action instead of joystick")
parser.add_argument("--no_sfc", action="store_true",
                    help="Disable CIRI safe flight corridors (MPC-only)")
parser.add_argument("--num_obstacles", type=int, default=60,
                    help="Number of obstacles in the tunnel terrain")
AppLauncher.add_app_launcher_args(parser)
args_cli = parser.parse_args()
app_launcher = AppLauncher(args_cli)
simulation_app = app_launcher.app

# --- 导入依赖 (必须在 SimulationApp 启动后导入) ---
import sys
import os
import torch
import numpy as np
import logging
from datetime import datetime
import omni.appwindow
import isaacsim.core.utils.prims as prim_utils
import isaaclab.sim as sim_utils
from isaaclab.sim import SimulationContext
from isaaclab.terrains import TerrainImporterCfg, TerrainImporter, TerrainGeneratorCfg
from isaaclab.sensors import RayCaster, RayCasterCfg, patterns
from isaacsim.core.utils.rotations import quat_to_euler_angles
from isaacsim.util.debug_draw import _debug_draw
from isaacsim.core.utils.viewports import set_camera_view

from omni_drones.robots.drone import MultirotorBase
from omni_drones.controllers import LeePositionController
from omni_drones.utils.torch import quat_rotate_inverse, quat_rotate

# IPC modules
sys.path.insert(0, os.path.dirname(__file__))
from ipc.sfc_visualizer import polytope_to_edges
from ipc.ipc_controller import IPCController, load_config

# Shared tunnel terrain utilities
from tunnel_terrain import (
    tunnel_obstacles_terrain, HfTunnelObstaclesTerrainCfg,
    INIT_POS, INIT_QUAT, TERRAIN_LEGACY_SEED,
    quat_to_rotation_matrix, extract_obstacles_from_heightfield,
    clear_captured_tiles,
)

# Joystick & UserModel
shared_demos_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../shared_demos"))
sys.path.insert(0, shared_demos_dir)
from joystick_wrapper import JoystickInterface
from srlc_model import MockConfig

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../isaac-training/src/core")))
from user_model_tunnely import UserModelTunnel

drone_model_name = "Hummingbird"
drone_controller_name = "LeePositionController"


def setup_logger(log_dir="logs"):
    os.makedirs(log_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = os.path.join(log_dir, f"verify_ipc_{timestamp}.log")

    logger = logging.getLogger("verify_ipc")
    logger.setLevel(logging.DEBUG)

    fh = logging.FileHandler(log_file)
    fh.setLevel(logging.DEBUG)
    fh.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))

    ch = logging.StreamHandler()
    ch.setLevel(logging.INFO)
    ch.setFormatter(logging.Formatter('%(levelname)s: %(message)s'))

    logger.addHandler(fh)
    logger.addHandler(ch)
    return logger, log_file


class Visualizer:
    """Identical to verify_srlc.py Visualizer — trajectory + velocity arrows."""

    def __init__(self, max_steps=200):
        self._draw = _debug_draw.acquire_debug_draw_interface()
        self.max_steps = max_steps
        self.history = []

    def update(self, pos, human_vel_w, ipc_vel_w):
        if isinstance(pos, torch.Tensor):
            pos = pos.detach().cpu().squeeze().tolist()

        self.history.append(pos)
        if len(self.history) > self.max_steps:
            self.history.pop(0)

        starts, ends, colors, sizes = [], [], [], []

        # Trajectory (Red)
        if len(self.history) >= 2:
            starts.extend(self.history[:-1])
            ends.extend(self.history[1:])
            colors.extend([(1.0, 0.0, 0.0, 1.0)] * (len(self.history) - 1))
            sizes.extend([2.0] * (len(self.history) - 1))

        # Human velocity (Green)
        if human_vel_w is not None:
            if isinstance(human_vel_w, torch.Tensor):
                human_vel_w = human_vel_w.detach().cpu().squeeze().tolist()
            end_h = [p + v for p, v in zip(pos, human_vel_w)]
            starts.append(pos)
            ends.append(end_h)
            colors.append((0.0, 1.0, 0.0, 1.0))
            sizes.append(4.0)

        # IPC output velocity (Blue)
        if ipc_vel_w is not None:
            if isinstance(ipc_vel_w, np.ndarray):
                ipc_vel_w = ipc_vel_w.tolist()
            end_m = [p + v for p, v in zip(pos, ipc_vel_w)]
            starts.append(pos)
            ends.append(end_m)
            colors.append((0.0, 0.0, 1.0, 1.0))
            sizes.append(4.0)

        self._draw.clear_lines()
        if starts:
            self._draw.draw_lines(starts, ends, colors, sizes)

    def update_sfc(self, sfc_dict: dict, ref_path=None):
        """Draw SFC corridors (cyan) and A* path (magenta).

        Args:
            sfc_dict: ``{step_index: (K,4) ndarray}`` from
                ``IPCController.get_last_sfc()``.
            ref_path: ``(M,3) ndarray`` A* reference path or None.
        """
        starts, ends, colors, sizes = [], [], [], []

        # Cyan wireframe for each SFC polytope
        for _step, planes in sfc_dict.items():
            edges = polytope_to_edges(planes)
            for s, e in edges:
                starts.append(s.tolist())
                ends.append(e.tolist())
                colors.append((0.0, 1.0, 1.0, 0.6))
                sizes.append(1.5)

        # Magenta A* reference path
        if ref_path is not None and len(ref_path) >= 2:
            for i in range(len(ref_path) - 1):
                starts.append(ref_path[i].tolist())
                ends.append(ref_path[i + 1].tolist())
                colors.append((1.0, 0.0, 1.0, 1.0))
                sizes.append(3.0)

        if starts:
            self._draw.draw_lines(starts, ends, colors, sizes)


def main():
    log_dir = os.path.join(os.path.dirname(__file__), "logs")
    logger, log_file = setup_logger(log_dir)
    logger.info(f"Log file: {log_file}")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    dt = 1.0 / 60.0

    sim_cfg = sim_utils.SimulationCfg(dt=dt, device=device)
    sim = SimulationContext(sim_cfg)

    # --- Drone ---
    drone, controller = MultirotorBase.make(
        drone_model_name, drone_controller_name, device
    )
    drone.spawn(translations=torch.tensor([INIT_POS], device=device))

    # --- Lighting ---
    cfg_light = sim_utils.DistantLightCfg(intensity=3000.0, color=(0.75, 0.75, 0.75))
    cfg_light.func("/World/Light", cfg_light)
    sky_light_cfg = sim_utils.DomeLightCfg(color=(0.2, 0.2, 0.3), intensity=2000.0)
    sky_light_cfg.func("/World/skyLight", sky_light_cfg)

    # --- Terrain (Tunnel with obstacles — same as verify_tunnel.py) ---
    terrain_cfg = TerrainImporterCfg(
        num_envs=1,
        env_spacing=0.0,
        prim_path="/World/ground",
        terrain_type="generator",
        terrain_generator=TerrainGeneratorCfg(
            seed=0,
            size=(24.0, 12.0),
            border_width=5.0,
            num_rows=1,
            num_cols=1,
            horizontal_scale=0.1,
            vertical_scale=0.1,
            slope_threshold=0.75,
            use_cache=False,
            color_scheme="height",
            curriculum=False,
            difficulty_range=(0.0, 1.0),
            sub_terrains={
                "obstacles": HfTunnelObstaclesTerrainCfg(
                    size=(1.0, 1.0),
                    horizontal_scale=0.1,
                    vertical_scale=0.1,
                    border_width=0.0,
                    num_obstacles=args_cli.num_obstacles,
                    obstacle_height_mode="choice",
                    obstacle_width_range=(0.4, 1.1),
                    obstacle_height_range=(8.0, 20.0),
                    platform_width=0,
                ),
            },
        ),
        visual_material=None,
        max_init_terrain_level=None,
        collision_group=-1,
        debug_vis=False,
    )
    clear_captured_tiles()
    np.random.seed(TERRAIN_LEGACY_SEED)
    terrain = terrain_cfg.class_type(terrain_cfg)

    # --- LiDAR (for visualization, not used by IPC) ---
    LIDAR_RANGE = 4.0
    LIDAR_VFOV = (-10.0, 20.0)
    LIDAR_VBEAMS = 4
    LIDAR_HRES = 10.0

    drone_prim_path = f"/World/envs/env_0/{drone.name}_0/base_link"
    ray_caster_cfg = RayCasterCfg(
        prim_path=drone_prim_path,
        offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 0.0)),
        ray_alignment="yaw",
        pattern_cfg=patterns.BpearlPatternCfg(
            horizontal_res=LIDAR_HRES,
            vertical_ray_angles=torch.linspace(LIDAR_VFOV[0], LIDAR_VFOV[1], LIDAR_VBEAMS)
        ),
        debug_vis=False,
        mesh_prim_paths=["/World/ground"],
    )
    lidar: RayCaster = ray_caster_cfg.class_type(ray_caster_cfg)
    logger.info(f"LiDAR initialized at {drone_prim_path}")

    sim.reset()
    drone.initialize()

    joystick = JoystickInterface()
    vis = Visualizer(max_steps=500)

    # --- UserModel ---
    user_model = None
    if args_cli.usermodel:
        mock_cfg = MockConfig(device)
        user_model = UserModelTunnel(num_envs=1, cfg=mock_cfg, logger=logger)
        logger.info("UserModelTunnel enabled")

    # --- IPC Controller ---
    config_path = args_cli.config
    if config_path is None:
        config_path = os.path.join(os.path.dirname(__file__), "ipc_config.yaml")

    ipc_cfg = load_config(config_path)
    if args_cli.no_sfc:
        ipc_cfg.use_sfc = False

    ipc = IPCController(cfg=ipc_cfg)

    # Build occupancy grid from terrain (prefer heightfield for accuracy)
    logger.info("Building occupancy grid from tunnel terrain...")
    hf, obstacles = extract_obstacles_from_heightfield(terrain_cfg.terrain_generator, tunnel_mode=True)
    gen_cfg = terrain_cfg.terrain_generator
    if hf is not None and hf.any():
        origin = np.array([-gen_cfg.size[0] / 2.0, -gen_cfg.size[1] / 2.0, 0.0])
        ipc.build_map(heightfield=hf.astype(float),
                      horizontal_scale=gen_cfg.horizontal_scale,
                      vertical_scale=gen_cfg.vertical_scale,
                      origin=origin)
        logger.info(f"Occupancy grid built from heightfield ({len(obstacles)} obstacle clusters)")
    elif obstacles:
        ipc.build_map(obstacles=obstacles)
        logger.info(f"Occupancy grid built with {len(obstacles)} obstacles")
    else:
        logger.warning("No obstacles extracted. MPC runs without occupancy grid.")

    # --- UserModel reset ---
    if user_model is not None:
        init_pos = torch.tensor([[-7.0, 0.0, 5.0]], device=device)
        init_quat = torch.tensor([[1.0, 0.0, 0.0, 0.0]], device=device)
        user_model.reset(init_pos, init_quat, torch.tensor([0], device=device))

    max_speed_xy = 2.0
    max_speed_z = 1.0
    max_yaw_rate = 0.5

    VIEWER_EYE_OFFSET = [-4.0, 0.0, 2.0]
    VIEWER_LOOKAT_OFFSET = [0.0, 0.0, 1.0]

    print("[INFO] Setup complete.")
    print(f"[INFO] Joystick: {joystick.connected}")
    print(f"[INFO] UserModel: {args_cli.usermodel}")
    print(f"[INFO] SFC: {not args_cli.no_sfc}")

    # Acceleration estimator (simple finite difference)
    prev_vel_w = np.zeros(3)

    # ===== Simulation Loop =====
    NUM_FRAMES = 20000
    frame_count = 0
    while simulation_app.is_running():
        if frame_count >= NUM_FRAMES:
            logger.info("Simulation finished.")
            break

        if sim.is_playing():
            # 1. Get drone state
            root_state = drone.get_state()[..., :13]  # (1, 1, 13)
            current_pos = root_state[..., :3]
            current_quat = root_state[..., 3:7]
            current_vel_w = root_state[..., 7:10]

            # Extract numpy arrays
            pos_np = current_pos.squeeze().cpu().numpy().astype(float)
            vel_np = current_vel_w.squeeze().cpu().numpy().astype(float)
            quat_np = current_quat.squeeze().cpu().numpy().astype(float)  # [w, x, y, z]

            # Estimate acceleration
            acc_np = (vel_np - prev_vel_w) / dt
            prev_vel_w = vel_np.copy()

            # Rotation matrix for body→world
            rot_mat = quat_to_rotation_matrix(quat_np)

            # 2. Get human action (body frame)
            if user_model is not None:
                vel_b = quat_rotate_inverse(current_quat.squeeze(1), current_vel_w.squeeze(1))
                ang_vel_w = root_state[..., 10:13]
                ang_vel_b = quat_rotate_inverse(current_quat.squeeze(1), ang_vel_w.squeeze(1))
                drone_state = torch.cat([vel_b, ang_vel_b, current_quat.squeeze(1)], dim=-1)
                drone_pos_w = current_pos.squeeze(1)
                human_action, _ = user_model.step(drone_state, drone_pos_w)
            else:
                stick_input = joystick.get_input().to(device)
                vel_cmd_x_body = -stick_input[1] * max_speed_xy
                vel_cmd_y_body = stick_input[0] * max_speed_xy
                vel_cmd_z = stick_input[2] * max_speed_z
                yaw_rate_cmd = -stick_input[3] * max_yaw_rate
                human_action = torch.tensor(
                    [[vel_cmd_x_body, vel_cmd_y_body, vel_cmd_z, yaw_rate_cmd]],
                    device=device
                )

            user_vel_body_np = human_action[..., :3].squeeze().cpu().numpy().astype(float)
            # UserModel returns (1, 3) — no yaw; joystick returns (1, 4) — with yaw
            if human_action.shape[-1] >= 4:
                human_yaw_rate = human_action[..., 3]
            else:
                human_yaw_rate = torch.zeros(1, device=device)

            # 3. IPC controller step
            ipc_vel_w = ipc.step(pos_np, vel_np, acc_np, user_vel_body_np, rot_mat)

            # 4. Apply control (same as verify_srlc.py)
            target_vel = torch.tensor(ipc_vel_w, dtype=torch.float32, device=device)
            target_vel = target_vel.reshape(1, 1, 3)
            target_yaw = human_yaw_rate.unsqueeze(1) * torch.pi  # Use human yaw

            action = controller(
                root_state=root_state,
                target_vel=target_vel,
                target_yaw=target_yaw,
            )
            drone.apply_action(action)

            # 5. Visualization
            eye_vel_offset = -current_vel_w * torch.tensor([2.0, 2.0, 0.0], device=device)
            set_camera_view(
                eye=current_pos.squeeze().cpu() + eye_vel_offset.squeeze().cpu() + torch.as_tensor(VIEWER_EYE_OFFSET).cpu(),
                target=current_pos.squeeze().cpu() + torch.as_tensor(VIEWER_LOOKAT_OFFSET).cpu(),
            )

            human_vel_b = human_action[..., :3]
            human_vel_w = quat_rotate(current_quat.squeeze(1), human_vel_b)
            vis.update(current_pos, human_vel_w, ipc_vel_w)
            vis.update_sfc(ipc.get_last_sfc(), ipc.get_ref_path())

            # Logging
            logger.debug(
                f"Frame {frame_count:06d} | Pos: [{pos_np[0]:7.3f}, {pos_np[1]:7.3f}, {pos_np[2]:7.3f}] | "
                f"IPC Vel: [{ipc_vel_w[0]:6.3f}, {ipc_vel_w[1]:6.3f}, {ipc_vel_w[2]:6.3f}] | "
                f"Human Vel (body): [{user_vel_body_np[0]:6.3f}, {user_vel_body_np[1]:6.3f}, {user_vel_body_np[2]:6.3f}]"
            )

        sim.step(render=True)
        frame_count += 1

    # Print SFC stats at exit
    sfc_stats = ipc.get_sfc_stats()
    print(f"\n{'='*50}")
    print(f"SFC Statistics:")
    print(f"  Attempts:     {sfc_stats['attempts']}")
    print(f"  Successes:    {sfc_stats['successes']}")
    print(f"  Success Rate: {sfc_stats['success_rate']:.1%}")
    print(f"{'='*50}")

    simulation_app.close()


if __name__ == "__main__":
    main()
