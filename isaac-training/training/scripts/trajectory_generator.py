...
def generate_single_trajectory_cpu(
    ...
    third_party_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../third_party')
    if third_party_path not in sys.path:
        sys.path.insert(0, third_party_path)
    
    try:
        from perlin_noise import PerlinNoise, Interp  # type: ignore
    except ImportError as e:
        raise ImportError(
            f"Could not import perlin_noise from {third_party_path}. "
            f"Make sure the third_party/perlin_noise.py file exists. Error: {e}"
        )
