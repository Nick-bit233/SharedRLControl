...
def main(cfg):
    ...
    # This experiment line intentionally implements only the Beta Lagrangian variant.
    algo_distribution = cfg.algo.get("distribution", "tanh_normal")
    if algo_distribution != "beta":
        raise ValueError("04s_tunnel_lagrangian requires algo.distribution=beta")
    ...
    def load_policy_state_dict(state_dict):
        try:
    ...
    if resume_state is not None:
        try:
            ...
            if "lambda_optimizer" in resume_state and hasattr(policy, "lambda_optimizer"):
                policy.lambda_optimizer.load_state_dict(resume_state["lambda_optimizer"])
