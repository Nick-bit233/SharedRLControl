"""
Constrained Residual PPO-Lagrangian with Beta Distribution.
...
Usage: swap import in train.py:
    from src.algos.ppo_constrained_beta_lagrangian import ConstrainedResidualPPO_BetaLagrangian as ConstrainedResidualPPO
"""
...
class ConstrainedResidualPPO_BetaLagrangian(TensorDictModuleBase):
    """Constrained Residual PPO-Lagrangian with Beta distribution.
    ...
    def _update(self, minibatch): 
        ...
        with profiler.timer("ppo/update/loss_compute"):
            ...
            # 3. Safety Cost Loss (Lagrangian or fixed-coefficient ablation)
            cost = minibatch[("next", "agents", "cost")]
            mean_cost = cost.mean()
            cost_surr1 = cost * ratio
            cost_surr2 = cost * ratio.clamp(1.-self.cfg.actor.clip_ratio, 1.+self.cfg.actor.clip_ratio)
            cost_policy_objective = torch.mean(torch.max(cost_surr1, cost_surr2))
            if self.use_lagrangian:
                ...
                safety_loss = cost_coeff * cost_policy_objective
                lambda_loss = torch.zeros((), device=self.device)

            # 4. Regularization Loss — penalize deviation from human action
            #    _mean_delta is the residual in [0,1] space (analogous to _loc_delta)
            reg_loss = minibatch["_mean_delta"].pow(2).sum(dim=-1).mean()

            # 5. Policy Loss
            loss_pi = actor_loss + safety_loss + self.reg_coeff * reg_loss
            
            # 6. Critic Loss (identical to Beta baseline)
        ...
        with profiler.timer("ppo/update/backward"):
            ...
            return TensorDict({
                ...
                "mean_cost": mean_cost,
                "cost_policy_objective": cost_policy_objective,
                "lambda_lag": self.lambda_lag.detach(),
    ...
    def _nan_info_dict(self):
        ...
        return TensorDict({
            ...
            "safety_loss": torch.tensor(float('nan')),
            "mean_cost": torch.tensor(float('nan')),
            "cost_policy_objective": torch.tensor(float('nan')),
            "lambda_lag": torch.tensor(float('nan')),
            "lambda_loss": torch.tensor(float('nan')),
...
ConstrainedResidualPPO_Beta = ConstrainedResidualPPO_BetaLagrangian
